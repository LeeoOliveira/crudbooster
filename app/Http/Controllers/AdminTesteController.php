...
class AdminTesteController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	        ...
	        */
			...
			$this->index_button[] = ['label'=>'Adicionar Produto','url'=>('/teste/add'),"icon"=>"fa fa-home"];
	    ...
	    */
		...
		public function getIndex() {
			//First, Add an auth
			 ...
			 $data['button_edit'] = $this->button_edit;
			 $data['button_add'] = $this->button_add;
			 $data['result'] = DB::table('produtos as p')
			 ->select('categorias.nome as nome_categoria','p.id', 'p.id_categorias', 'p.nome', 'p.preco', 'p.descricao')
			 ->leftjoin('categorias', 'p.id_categorias', '=', 'categorias.id')
			 ->orderby('categorias.id','desc')
			 ->paginate(10);
			//  dd($data['result']);			  
			//   dd(CRUDBooster::mainPath(), CRUDBooster::adminPath());
		  ...
		  public function getAdd() {
			...
			$data = [];
			$data['page_title'] = 'Adicionar produtos';
			$data['dados'] = DB::select('select c.nome as nome_categoria from produtos as p join categorias as c on c.id = p.id_categorias');
